...
const DocumentViewer: React.FC<DocumentViewerProps> = ({ doc, title, colorTheme, highlightText }) => {
  ...
  useEffect(() => {
    // Trigger re-render or side-effect if needed when highlightText changes
    // The actual highlighting is done in TextView render
  }, [highlightText]);
  ...
  const TextView = () => {
    const content = doc?.text || "No text content available.";
    ...
    if (highlightText && content.includes(highlightText)) {
      ...
      renderedContent = parts.map((part, i) => (
        <React.Fragment key={i}>
          {part}
          {i < parts.length - 1 && (
            <span className="bg-yellow-200 text-gray-900 font-bold px-1 rounded animate-pulse" ref={(el) => {
              if (el && i === 0) {
    ...
    return (
      <div className="h-full overflow-y-auto p-6 scroll-smooth bg-white">
        <pre ref={textRef} className="whitespace-pre-wrap font-mono text-sm text-gray-800 leading-relaxed font-normal">
          {renderedContent}
        </pre>
      </div>
    );
...
export const PreviewModal: React.FC<PreviewModalProps> = ({ file, onClose, jobSpec }) => {
  const [summary, setSummary] = useState<string | null>((file as any).summary || null);
  const [isGenerating, setIsGenerating] = useState(false);
  const [summaryStyle, setSummaryStyle] = useState<'bullet' | 'narrative'>('bullet');
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      const result = await generateSummary(file.text, summaryStyle);
      setSummary(result);
      // Update local file object temporarily to persist within session
      (file as any).summary = result;
    } catch (e) {
  ...
  const handleSourceClick = (text: string) => {
    // Clean up text to improve matching success
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl transform transition-all w-full max-w-[95vw] h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-white shrink-0">
            <div className="min-w-0">
              <h3 className="text-xl font-bold text-gray-900 truncate flex items-center gap-3">
                {file.name}
                {'score' in file && (
                  <span className={`text-sm px-3 py-1 rounded-full font-bold border shrink-0 ${(file as CandidateResult).score >= 0.7 ? 'bg-green-100 text-green-700 border-green-200' :
          ...
          <div className="flex-1 bg-gray-50 overflow-hidden p-4 sm:p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 h-full">
              ...
              {/* Left Panel: Candidate Document & AI Insight */}
              <div className="flex flex-col h-full gap-4">
                ...
                {/* AI Summary Section */}
                <div className="bg-white rounded-xl shadow-sm border border-indigo-100 flex flex-col overflow-hidden h-[40%] shrink-0">
                  <div className="px-4 py-2 bg-indigo-50 border-b border-indigo-100 flex justify-between items-center shrink-0">
                    ...
                    <div className="flex bg-white rounded-lg p-0.5 border border-indigo-100">
                      <button
                        ...
                        className={`px-2 py-0.5 text-xs font-medium rounded transition-colors ${summaryStyle === 'bullet' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-indigo-600'}`}
                      >
                        Bullets
                      </button>
                      <button
                        onClick={() => setSummaryStyle('narrative')}
                        className={`px-2 py-0.5 text-xs font-medium rounded transition-colors ${summaryStyle === 'narrative' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-indigo-600'}`}
                      >
                        Narrative
                      </button>
                    </div>
                  </div>
                  <div className="p-4 overflow-y-auto flex-1 bg-gradient-to-br from-indigo-50/30 to-purple-50/30">
                    {!summary ? (
                      <div className="flex flex-col items-center justify-center h-full text-center space-y-3">
                        <p className="text-sm text-gray-500">Generate a summary to quickly understand this candidate's fit.</p>
                        <button
                          onClick={handleGenerateSummary}
                          disabled={isGenerating}
                          className="px-4 py-2 bg-white border border-indigo-200 text-indigo-700 rounded-lg text-sm font-medium shadow-sm hover:bg-indigo-50 hover:shadow transition-all disabled:opacity-50"
                        >
                          {isGenerating ? (
                      ...
                      <div className="prose prose-sm prose-indigo max-w-none">
                        <div className="flex justify-end mb-2">
                          <button onClick={handleGenerateSummary} className={`text-xs text-indigo-500 hover:text-indigo-700 underline ${isGenerating ? 'opacity-50 cursor-wait' : ''}`}>
                            {isGenerating ? 'Regenerating...' : 'Regenerate'}
                          </button>
                        </div>
                        <div
                          className="text-gray-700 text-sm leading-relaxed cursor-text selection:bg-indigo-100"
                          onClick={(e) => {
                            const selection = window.getSelection();
                            if (selection && selection.toString().length > 5) {
                              handleSourceClick(selection.toString());
                            }
                          }}
                        >
                          {summary.split('\n').map((line, i) => (
